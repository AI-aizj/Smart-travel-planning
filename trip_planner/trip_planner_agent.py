...
from trip_planner.prompts定义各智能体的提示词模板 import WEATHER_AGENT_PROMPT, ATTRACTION_AGENT_PROMPT, \
    HOTEL_AGENT_PROMPT, PLANNER_AGENT_PROMPT
from trip_planner.schemas定义所有数据模型 import TripRequest, TripPlan
...
class MultiAgentTripPlanner:
    ...
    async def initialize(self):
        ...
        # 检查是否有有效的LLM
        if not self.llm:
            print("  - 错误: 未配置有效的LLM，无法初始化多智能体系统")
            return False
        try:
            print("  - 创建共享MCP工具...")
            # 检查是否有有效的高德地图API密钥
            if not AMAP_API_KEY or AMAP_API_KEY.strip() == "":
                print("  - 警告: 未配置高德地图API密钥，将使用模拟数据")
                self.amap_tool = None
                tools = []
            else:
                try:
                    ...
                    print(f"  - 警告: 连接高德地图API失败 ({str(e)})，将使用模拟数据")
                    self.amap_tool = None
                    tools = []
    ...
    async def plan_trip(self, request: TripRequest) -> TripPlan:
        ...
        # 检查是否成功初始化
        if not self.llm:
            raise ValueError("未配置有效的LLM，无法进行旅行规划。请在.env文件中配置DASHSCOPE_API_KEY")
        try:
            print(f"\n{'='*60}")
...
async def main():
    ...
    try:
        ...
        print("\n✅ 生成的旅行计划：")
        print(trip_plan.model_dump_json(indent=2))  # 打印JSON
    except Exception as e:
        print(f"规划失败: {e}")
